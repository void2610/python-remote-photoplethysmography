#!/usr/bin/env python3
"""
動画ファイルデモ

動画ファイルから心拍数を測定します。
"""

import sys
import argparse
from pathlib import Path

# プロジェクトルートをパスに追加
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))

from src.main import HeartRateMonitor


def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(
        description='動画ファイルから心拍数測定'
    )

    parser.add_argument(
        '--input',
        type=str,
        required=True,
        help='入力動画ファイルパス'
    )

    parser.add_argument(
        '--algorithm',
        type=str,
        default='green',
        choices=['green'],
        help='rPPGアルゴリズム (デフォルト: green)'
    )

    parser.add_argument(
        '--buffer-size',
        type=int,
        default=300,
        help='信号バッファサイズ (デフォルト: 300フレーム)'
    )

    parser.add_argument(
        '--estimation-method',
        type=str,
        default='fft',
        choices=['fft', 'peaks', 'autocorr', 'cwt'],
        help='心拍数推定手法 (デフォルト: fft)'
    )

    parser.add_argument(
        '--save-video',
        type=str,
        default=None,
        help='動画保存パス (例: output.mp4)'
    )

    parser.add_argument(
        '--no-visualization',
        action='store_true',
        help='可視化を無効にする'
    )

    args = parser.parse_args()

    # 入力ファイルの確認
    input_path = Path(args.input)
    if not input_path.exists():
        print(f"エラー: 入力ファイルが見つかりません: {args.input}")
        sys.exit(1)

    # モニター初期化
    print("=" * 60)
    print("リモート心拍数測定システム - 動画ファイルデモ")
    print("=" * 60)
    print(f"入力動画: {args.input}")
    print(f"アルゴリズム: {args.algorithm.upper()}")
    print(f"バッファサイズ: {args.buffer_size}フレーム")
    print(f"推定手法: {args.estimation_method}")
    if args.save_video:
        print(f"動画保存: {args.save_video}")
    print("=" * 60)
    print("\n操作方法:")
    print("  'q' キー: 終了")
    print("=" * 60)
    print()

    monitor = HeartRateMonitor(
        algorithm=args.algorithm,
        buffer_size=args.buffer_size,
        estimation_method=args.estimation_method
    )

    try:
        monitor.run_video_file(
            video_path=args.input,
            show_visualization=not args.no_visualization,
            save_video=args.save_video
        )
    except KeyboardInterrupt:
        print("\n\nキーボード割り込みにより終了します...")
    except Exception as e:
        print(f"\nエラーが発生しました: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)

    # 最終結果を表示
    status = monitor.get_status()
    print("\n" + "=" * 60)
    print("測定結果")
    print("=" * 60)
    print(f"処理フレーム数: {status['frame_count']}")
    print(f"最終心拍数: {status['last_heart_rate']:.1f} BPM")
    print(f"信頼度: {status['last_confidence']:.2f}")
    print("=" * 60)


if __name__ == "__main__":
    main()
